...
Generated by `python scripts/dump_prompts.py` from the live prompt builders in `app/rules.py` and `app/generator.py`. Do not edit by hand. Rules version 0.1.0.
...
## Rulebook (version 0.1.0, 2026-09-05)
...
### T5 — Celebrity or influencer as authority  [TONE · NOTE]
...
What Minimalist does instead: 'evaluated for safety through patch testing under the supervision of a Dermatologist'; supplier provenance.
Fix pattern: Replace with the study result or the dermatologist patch-test statement.

### T6 — Transformation narrative  [TONE · NOTE]
...
What Minimalist does instead: Observable, specific descriptors: 'skin looks clear', 'skin felt less oily throughout the day'.
Fix pattern: Describe the observable change and its timeframe.
...
### L3 — Result claim without timeframe  [LANGUAGE · NOTE]
...
What Minimalist does instead: 'improve skin complexion in 2 weeks', 'reduce number of blackheads by 50% after 28 days', 'visible skin clarity in 4 weeks'.
Fix pattern: Add the timeframe the product page states. If none exists, leave the claim hedged and unquantified.
...
### L5 — 'Natural' / 'herbal' / 'organic' as a virtue  [LANGUAGE · NOTE]
